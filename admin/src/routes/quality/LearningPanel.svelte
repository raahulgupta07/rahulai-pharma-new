<script>
  ...
  // The loaders go through $lib/api.js::getJSON, which throws an ApiError
  // carrying the status. The panel keeps the error OBJECT, not a string, so
  // ErrorState can tell an expired session from a stopped backend.
...
<div class="flex items-center gap-3">
  <div class="inline-flex rounded-card bg-surface-2 p-[3px]">
    {#each [['memories', 'Memories', Brain], ['feedback', 'Feedback', MessageSquare]] as [id, label, Icon]}
      <button
        onclick={() => (view = id)}
        class="flex cursor-pointer items-center gap-1.5 rounded-card px-4 py-1.5 text-body-sm font-semibold transition-colors
  ...
  <!--
    A count is printed only when the list it counts actually loaded. Falling
    back to `mRows.length` while the request is failing prints "0 memories" next
    to the failure, which reads as an empty store rather than as an unread one.
  -->
  <span class="text-meta text-ink-3">
    {#if view === 'memories'}
      {mError ? '' : `${fmt(lStats?.total ?? mRows.length)} memories`}
    {:else}
      {fError ? '' : `${fmt(fStats?.total ?? fRows.length)} responses rated`}
    {/if}
  </span>
</div>
...
{#if view === 'memories'}
  <!-- MEMORIES table -->
  <div class="my-3.5 elev overflow-hidden rounded-hero border border-line bg-surface">
        ...
        <tbody>
          ...
          {:else if mError}
            <tr>
              <td colspan="5" class="p-3">
                <ErrorState error={mError} retry={loadMemories} what="learned memories" />
              </td>
            </tr>
          ...
          {:else}
            {#each mRows as r (r.id)}
              ...
              <tr>
                <td><span class="rounded-control px-2 py-0.5 text-micro font-semibold {tb.cls}">{tb.txt}</span></td>
                <td class="font-mono text-meta text-ink-2">{r.user_id ?? '—'}</td>
                <td class="text-ink" title={r.summary}>{truncate(r.summary)}</td>
                <td class="text-meta text-ink-2" title={r.updated_at}>{relTime(r.updated_at)}</td>
                <td>
                  <button
                    onclick={() => forget(r.id)}
                    aria-label="Forget this memory"
                    title="Forget"
                    class="flex h-8 w-8 cursor-pointer items-center justify-center rounded-panel text-ink-3 transition-colors hover:bg-danger-soft hover:text-danger"
                  >
                    <Trash2 size={15} />
                  </button>
                </td>
        ...
        <tbody>
          ...
          {:else if fError}
            <tr>
              <td colspan="6" class="p-3">
                <ErrorState error={fError} retry={loadFeedback} what="chat feedback" />
              </td>
            </tr>
          ...
          {:else}
            {#each fFiltered as r (r.id)}
              <tr>
                <td class="text-meta text-ink-2" title={r.ts}>{shortWhen(r.ts)}</td>
                <td>
                  {#if r.verdict === 'up'}
                    <span class="inline-flex items-center gap-1 rounded-control bg-success-soft px-2 py-0.5 text-micro font-semibold text-success"><ThumbsUp size={11} /> Up</span>
                  {:else}
                    <span class="inline-flex items-center gap-1 rounded-control bg-danger-soft px-2 py-0.5 text-micro font-semibold text-danger"><ThumbsDown size={11} /> Down</span>
                  {/if}
                </td>
                <td class="font-mono text-label text-ink-2">{r.model ?? '—'}</td>
                ...
                <td class="font-mono text-meta text-ink-2">{r.store_id ?? '—'}</td>
              </tr>
            {/each}
          {/if}
        </tbody>
