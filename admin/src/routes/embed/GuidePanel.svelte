...
<div class="mb-5 flex flex-wrap gap-1.5">
  {#each tabs as t (t.id)}
    <button
      onclick={() => (tab = t.id)}
      class="rounded-panel border px-3 py-1.5 text-body-sm font-medium transition-colors {tab === t.id
...
{#snippet codeblock(key, code)}
  <div class="relative">
    <button
      ...
      class="absolute right-2 top-2 inline-flex items-center gap-1.5 rounded-panel border border-line bg-surface px-2.5 py-1 text-meta text-ink-2 transition-colors hover:bg-surface-2"
    >
      {#if copied === key}<Check size={13} /> Copied{:else}<Copy size={13} /> Copy{/if}
    </button>
    <pre
      class="overflow-x-auto rounded-panel border border-line bg-surface-2 p-4 pr-20 text-meta leading-relaxed text-ink"><code
        >{code}</code
      ></pre>
  </div>
{/snippet}
...
{#snippet card(title, children)}
  <section class="mb-5 rounded-panel border border-line bg-surface p-5">
    <h3 class="mb-3 text-body font-semibold text-ink">{title}</h3>
    {@render children()}
  </section>
{/snippet}
...
{#if tab === 'overview'}
  {#snippet overviewBody()}
    <p class="mb-4 text-body-sm leading-relaxed text-ink-2">
    ...
    <div class="grid gap-3 sm:grid-cols-3">
      <div class="rounded-card border border-line bg-surface-2 p-4">
        ...
        <p class="text-meta leading-relaxed text-ink-2">
...
{#if tab === 'outlet'}
  {#snippet outletA()}
    ...
    <ol class="ml-4 list-decimal space-y-2 text-body-sm text-ink-2">
    ...
    <p class="mt-3 text-meta text-ink-3">A floating chat bubble appears bottom-right. It only ever answers for your store.</p>
  {/snippet}
  ...
  {#snippet outletB()}
    <p class="mb-3 text-body-sm leading-relaxed text-ink-2">
      ...
      <a href={appBase + '/embed?tab=widget'} class="text-accent hover:underline">Embed widget</a> page.
    </p>
...
{#if tab === 'operator'}
  {#snippet opBody()}
    <ol class="ml-4 list-decimal space-y-3 text-body-sm text-ink-2">
      <li>
      ...
      <li>
        ...
        <a href={appBase + '/embed?tab=widget'} class="text-accent hover:underline">Embed widget</a>, enter the
      ...
      <li>
        ...
        <a href={appBase + '/settings?tab=behaviour'} class="text-accent hover:underline">Answer behaviour</a>, pick
...
{#if tab === 'devops'}
  {#snippet awsBody()}
    <ol class="ml-4 list-decimal space-y-2.5 text-body-sm text-ink-2">
  ...
  {#snippet endpointsBody()}
    <ul class="space-y-1 text-meta text-ink-2">
...
{#if tab === 'trouble'}
  {#snippet tbody()}
    <div class="space-y-4">
      <div>
        <p class="mb-1 flex items-center gap-2 text-body-sm font-medium text-ink"><Badge tone="danger">CORS</Badge> "blocked by CORS policy" / "Failed to fetch"</p>
        <p class="text-body-sm text-ink-2">The site's origin is not allowed. Add its exact origin (scheme + host + port, no path) on <a href={appBase + '/tenants'} class="text-accent hover:underline">Tenants → Allowed origins</a>. Takes effect in seconds.</p>
      </div>
      <div>
        <p class="mb-1 flex items-center gap-2 text-body-sm font-medium text-ink"><Badge tone="danger">403</Badge> "invalid embed credentials"</p>
        <p class="text-body-sm text-ink-2">The <span class="font-mono">embed_id</span>/<span class="font-mono">public_key</span> in the snippet is not registered (or is the dev-only <span class="font-mono">web</span>/<span class="font-mono">web</span> in production). Register a real credential on Tenants and regenerate the snippet.</p>
      </div>
      <div>
        <p class="mb-1 flex items-center gap-2 text-body-sm font-medium text-ink"><Badge tone="danger">401</Badge> "bad user signature"</p>
        <p class="text-body-sm text-ink-2">The signing secret does not match the backend, or the signed user object was altered. Confirm <span class="font-mono">SECRET_KEY</span> equals the signer's key. Downloaded outlet snippets are pre-signed — do not edit the <span class="font-mono">data-user</span> / <span class="font-mono">data-user-sig</span> values.</p>
      </div>
      <div>
        <p class="mb-1 flex items-center gap-2 text-body-sm font-medium text-ink"><Badge tone="warn">stale</Badge> answer is out of date</p>
        <p class="text-body-sm text-ink-2">The answer cache (up to 10 min) held an old reply. Call <span class="font-mono">POST /api/embed/reload</span> after a data change or deploy.</p>
      </div>
      <div>
        <p class="mb-1 flex items-center gap-2 text-body-sm font-medium text-ink"><Badge tone="warn">blank</Badge> no bubble appears</p>
        <p class="text-body-sm text-ink-2">Check the browser console. Usually a CORS block (above) or the widget script 404'd — confirm <span class="font-mono">{cleanBase}/api/embed/widget.js</span> loads.</p>
      </div>
    </div>
  {/snippet}
