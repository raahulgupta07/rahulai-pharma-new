<script>
  ...
  import { Copy, Check, TriangleAlert, ExternalLink } from '@lucide/svelte';
  ...
  async function loadCreds() {
    ...
    credsError = null;
    try {
      // getJSON keeps the status on the error, so an expired token reads as an
...
<script src="<?= htmlspecialchars($CITYAGENT_BASE) ?>/api/embed/widget.js"
  ...
  // The LIST's failure is kept as the thrown ApiError (so ErrorState can read
  // its status); `outletError` stays a plain string for the generate/download
  // actions below, which report a `detail` the backend wrote.
...
>
  {#snippet actions()}
    <!-- This page emits snippets; it does not run a widget. A live widget runs
         in exactly one place — Branch assistant — where it is framed, sandboxed
         and explained. This page used to run a second one by appending
         widget.js to the CONSOLE's own body: the launcher is position:fixed and
         the SPA never reloads, so the bubble outlived this page, followed the
         operator across every route, and a second click added another. A link
         cannot do that. -->
    <a
      href={appBase + '/widget'}
      class="inline-flex items-center gap-2 rounded-panel bg-accent px-3.5 py-2 text-body-sm font-medium text-on-accent transition-colors hover:bg-accent-hover"
    >
      See it running → Branch assistant
    </a>
  {/snippet}
</PageHeader>

<!-- 1. Public backend URL -->
<section class="mb-6 rounded-panel border border-line bg-surface p-4">
  <div class="mb-1 flex items-center gap-2">
    <span id="public-backend-url" class="text-body-sm font-medium text-ink">Public backend URL</span>
  ...
  <input
    ...
    aria-labelledby="public-backend-url"
    spellcheck="false"
    placeholder="https://agent.example.com"
    class="w-full rounded-panel border border-line bg-surface-2 px-3 py-2 font-mono text-body-sm text-ink outline-none focus:border-accent"
  />
  {#if isLocal}
...
<section class="mb-6 rounded-panel border border-line bg-surface p-4">
  <div class="mb-1 flex items-center gap-2">
    <span class="text-body-sm font-medium text-ink">Embed credential</span>
    ...
    <div class="flex flex-wrap items-center gap-2">
      {#each creds as c (c.embed_id)}
        <button
          ...
          class="rounded-panel border px-3 py-1.5 font-mono text-meta transition-colors {selected ===
          c.embed_id
...
<section class="mb-6 rounded-panel border border-line bg-surface p-4">
  <div class="mb-1 flex items-center gap-2">
    <span class="text-body-sm font-medium text-ink">Per-outlet embeds (download &amp; go)</span>
    <!-- Never a count derived from a list that failed to load: "0 stores" next
         to a failure reads as an empty estate rather than an unread one. -->
    {#if !outletsError && !outletsLoading}
  ...
  {#if !hasCred}
    <p class="flex items-start gap-1.5 text-body-sm text-warning">
      <TriangleAlert size={14} class="mt-0.5 shrink-0" />
  ...
  {:else}
    <div class="flex flex-wrap items-end gap-2">
      <label class="flex flex-col gap-1">
        <span class="text-meta text-ink-3">Store</span>
        <select
          bind:value={outletSel}
          class="min-w-[220px] rounded-panel border border-line bg-surface-2 px-3 py-2 font-mono text-body-sm text-ink outline-none focus:border-accent"
        >
      ...
      <button
        ...
        class="rounded-panel border border-line px-3 py-2 text-body-sm text-ink-2 transition-colors hover:bg-surface-2 disabled:opacity-60"
      >
      ...
      <button
        ...
        disabled={outletBusy || !outletSel}
        class="rounded-panel bg-accent px-3 py-2 text-body-sm font-medium text-on-accent transition-colors hover:bg-accent-hover disabled:opacity-60"
      >
      ...
      <button
        ...
        class="rounded-panel border border-accent px-3 py-2 text-body-sm font-medium text-accent-hover transition-colors hover:bg-accent-soft disabled:opacity-60"
      >
    ...
    {#if outletError}
      <p class="mt-3 text-body-sm text-danger">{outletError}</p>
    {/if}

    {#if outletSnippet}
      <div class="mt-3">
        <div class="mb-2 flex items-center">
          <span class="text-body-sm font-medium text-ink">Snippet for {outletSel}</span>
          <button
            onclick={() => copy('outlet', outletSnippet)}
            class="ml-auto inline-flex items-center gap-1.5 rounded-panel border border-line px-2.5 py-1 text-meta text-ink-2 transition-colors hover:bg-surface-2"
          >
...
{#snippet block(key, label, code, note)}
  <section class="mb-6">
    <div class="mb-2 flex items-center">
      <span class="text-body-sm font-medium text-ink">{label}</span>
      <button
    ...
    {#if note}<p class="mb-2 text-body-sm text-ink-2">{note}</p>{/if}
    <pre
      class="overflow-x-auto rounded-panel border border-line bg-surface-2 p-4 text-meta leading-relaxed text-ink"><code
        >{code}</code
      ></pre>
...
<div class="rounded-panel border border-line bg-surface p-4 text-body-sm text-ink-2">
  ...
  <div class="mt-1 text-ink-3">
    Full guide in INTEGRATION.md. To watch the widget answer real questions, open
    <a href={appBase + '/widget'} class="text-accent hover:underline">Branch assistant</a> — it runs
    this backend ({API_BASE}) inside a framed, store-pinned preview.
  </div>
</div>
