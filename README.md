...
**Accuracy is unmeasured.** `evals/bench.py` times answers; it does not grade
them. The accuracy eval (`evals/run_eval.py`, gated behind `RUN_LIVE=1`) has never
been run here, so no per-question pass/fail record exists. The fast path, the
`FORMATTING` prompt block, and `NULLS LAST` all change what the model says, and
none has been graded — the fast path especially, since it resolves a drug by
trigram with no LLM and can confidently return the wrong product.
