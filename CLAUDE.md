...
## ⚠️ Behind TLS: uvicorn needs `--proxy-headers` or the widget vanishes

`docker/Dockerfile` runs uvicorn with `--proxy-headers --forwarded-allow-ips "$FORWARDED_ALLOW_IPS"`.
Both halves are load-bearing and their absence is invisible until a browser refuses to run something.

Starlette builds `request.base_url` from the scheme uvicorn sees on the wire. Without the flag that is
always `http`, even when the proxy terminated TLS — so `GET /embed/preview` emitted
`<script src="http://…/api/embed/widget.js">` onto an https page and Chrome blocked it as mixed content.
The API answered 200 throughout and the page rendered; only the chat widget was missing, which reads as a
widget bug rather than a deploy one. The proxy was never at fault: it already sent
`X-Forwarded-Proto`; the app discarded it.

`--forwarded-allow-ips` is the trust boundary and is deliberately NOT `*` — the container port is
published on the host, so anything that can reach it could otherwise assert its own scheme and client IP.
It is an env var because the trusted peer is topology-specific: **where the proxy dials the host's PUBLIC
address, the peer is that public IP, not a private one** (measured: proxied requests arrived from
`52.220.37.93`, the box's own EIP, because the packet leaves via `ens5` and hairpins back through the
AWS gateway).

## ⚠️ `outline-none` is inert in this codebase — all 48 of them

`admin/src/app.css` is UNLAYERED; Tailwind utilities live in `@layer utilities`. Unlayered wins, so the
global `:where(…):focus-visible { outline: 2px … }` beats every `outline-none` in `admin/src`. Any
"suppress the ring here" written as a utility does nothing.

That produced a whole defect class — a **shell** control (a bordered box with a borderless input inside
it) drew a second ring INSIDE its own edge. Found on the chat composer, the header search, and both
sign-in fields, in both themes. Two classes now exist:

* `.field-shell` — suppression only. For a box that already tints its own border on focus (the sign-in
  fields carry `focus-within:border-accent` plus a `ring-4` glow; a hard outline there is the third edge).
* `.field-shell-ring` — suppression PLUS a 2px ring, for shells whose only signal is a 1px border.

Anything carrying `-ring` must also carry `.field-shell`. Suppression must be at least as specific AND
unlayered — a utility cannot do it.

**Tailwind v4 also dropped `cursor: pointer` from its button reset.** Measured before the fix: `/admin/`
5 controls, `/admin/chat` 8, `/admin/data` 12, `/admin/users` 4 — Sign out, Toggle theme, Reload and
Create among them — all showing an arrow. `app.css` restores it for `button, [role=button], summary`,
with `:disabled` kept as `default` so "you cannot press this" still reads.

## ⚠️ A malformed CSS comment drops the next rule, silently

Editing a comment in `app.css` and leaving the old `*/` in place produced:

```
   …the inner field has none. */      <- comment ended here
   Two classes, because two shells…   <- now raw CSS
   …must also carry `.field-shell`. */ <- stray close
.field-shell > input:focus-visible,   <- swallowed by the parse error
```

The build said `✔ done`, the page rendered, and exactly one rule was missing from the output. Nothing
errored. **After editing app.css, grep the BUILT css for the rule you just wrote** — `admin/build/_app/
immutable/assets/*.css`. A comment-balance check is two lines of Python and worth it.

## ⚠️ The embed preview shares a document with the widget

`/embed/preview` renders the demo page AND the widget in one document, so a bare element selector in
`_demo_page`'s inline CSS repaints the ASSISTANT'S OWN ANSWERS. An unscoped

```css
ul{list-style:none;display:flex} li{…;font-family:ui-monospace}
```

turned a five-bullet answer into a row of grey monospace pills inside the chat panel. The markdown was
correct the whole time — verified by running the real answer through `tests/js/widget_harness.js`, which
returned `<ul><li>…` with 0 `<pre>`. Every content rule there is now scoped under `.demo`. **Do not
reintroduce a bare element selector in that page.** (A customer's own site can do this to us too; the
widget scopes everything under `cca-` but does not defend against a host page's bare selectors.)

## ⚠️ The SFTP address: localStorage on one side, env on the other

`sftp_public_host` is env-only (`app/config.py`) with NO server-side write path. The address field on
`/admin/ftp` writes to **localStorage** — it persists to that browser, not to the server. So:

* the handover TEXT can legitimately use a typed address (it is text the operator is about to send);
* anything the SERVER builds — the ZIP packs, the one-file script — resolves the address itself and
  refuses while `SFTP_PUBLIC_HOST` is unset.

Gating both on one flag let the UI enable buttons the server would 400. Measured: with a hostname in
localStorage the pack controls went live while `/admin/sftp/connection` still reported
`host_source: "detected"`, `host_configured: false`. The page now has TWO derived guards —
`handoverBlocked` (usable-looking address) and `packBlocked` (`handoverBlocked || !envHost`, i.e. the
server knows it). Server-side refusal exists as well; the UI is never the only check.

Related: `h` (the address printed into every generated command) shouts `SFTP_HOST_NOT_SET` whenever
`handoverBlocked`. Its original condition was `hostKnown`, and **`localhost` is not empty**, so every
command on the page happily printed `pharma@localhost`. Copy was disabled, but the text sits on screen
and gets selected by hand.

## ⚠️ Partner keys: what is stored, what is not, and the atomic swap

`POST /admin/sftp/keys` (paste a public key), `/generate` (we make the pair), `/{label}/regenerate`
(replace in place), `DELETE /{label}`. Plus `/partner-pack` (ZIP) and `/partner-script` (one `.sh`).

* **A private key is never stored, never logged, never returned twice.** It exists in one response body
  and nowhere else. `_generated_keypair` touches no file, logger or pool. No middleware captures response
  bodies — checked — so it cannot reach the audit trail. This is why an existing partner's script comes
  back with a PLACEHOLDER key block: we genuinely cannot rebuild their file. Regenerate is the only path.
* **Order is load-bearing**: validate → check mount → generate → register → respond. A failed
  registration must return the error and NO private key; a secret for a key that was never installed
  buys nobody anything.
* **Regenerate is ONE lock acquisition and ONE `_atomic_write`.** Not delete-then-add: that is two
  rewrites with a window where `authorized_keys` holds neither key, and sshd reads that file on every
  connection. `keys/<label>.pub` is written first (atmoz/sftp rebuilds authorized_keys from it at boot),
  so a partial failure fails towards the partner KEEPING access.
* `activity._ROUTES` has no entry for the new sftp routes, so the audit middleware records route+status
  with an empty `detail` and loses the label. These endpoints record the event explicitly. Audit carries
  label + fingerprint(s) — public facts — never the secret.
* `cryptography` is now DECLARED in `requirements.txt`. It was only transitive; a future bump of whatever
  pulled it in would have taken key generation with it.

## ⚠️ A ZIP cannot carry a private key's permissions

Measured both ways:

```
zf.writestr(...)                          -> extracted 0o644
ZipInfo with external_attr = 0o600 << 16  -> extracted 0o644
```

Python's `extractall` ignores the mode bits and most GUI unzippers do too. Without `chmod 600` the
partner's first command dies with "Permissions 0644 … are too open." So **`chmod 600` lives in the
script**, not in the README — `setup.sh` and `send.sh` both do it defensively.

## ⚠️ Generated artefacts must reflect SETTINGS, not constants

`send.sh` originally ended `"picked up automatically; there is nothing else to do."` — hardcoded. When
`cache.get_ingest_enabled()` is false a file sits as Waiting until somebody presses Load, and the partner
is the one party who cannot open this console to find out. The line is now built from the real setting.
The same contradiction existed on the page between its status strip and its stale-file note.

**A zip or a script is worse than a screen for this**: it is kept and re-read for months after the
setting has moved.

## Notes from the 2026-08-20 session

* `/ftp` is no longer a tabbed page — five tabs became one page (measured: 1,948 words / 80 controls
  across the tabs, 1,062 of those words on "How to connect" alone, with 14 "cannot/never/wrong" warnings
  on that one tab). `tests/test_admin_tabbed_pages.py` discovers shells by scanning for `<TabStrip`, so
  the suite legitimately drops from **1315 to 1313** — the two `/ftp` parametrisations, nothing else.
  Diff collected test IDs before assuming a guard was lost.
* The Catalog table's "Sites" column was `<td class="num tnum">53</td>` — a **hardcoded literal** on all
  5,292 rows, including stubs, reading as "stocked in 53 branches". Removed; the real per-article
  `site_count` lives in the detail drawer where it is computed.
* Branding is DB-backed and product-name changes must follow it: `widget.js` only falls back to the brand
  name when `data-title` is ABSENT, and `_snippet_html` always emits one — so a hardcoded preview title
  survived a rename. Send the brand name, and omit the key entirely rather than sending `''` (falsy →
  `"Pharmacy · <store>"`, a third name for the same thing).
* `_demo_page(auto_open=…)` defaults **False** on purpose. Its output reaches customers in two places
